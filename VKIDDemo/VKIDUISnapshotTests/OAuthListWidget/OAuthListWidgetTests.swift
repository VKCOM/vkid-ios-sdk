...
final class OAuthListWidgetTests: XCTestCase, TestCaseInfra {
    ...
    func testConfiguration() {
        ...
        var oAuthListWidgetConfig: OAuthListWidget!
        let expectation = expectation(description: #function)
        given("Конфигурация виджета, имитация авторизации") {
            ...
            oAuthListWidgetConfig = OAuthListWidget(
                oAuthProviders: [.vkid],
        ...
        when("Нажатие на кнопку OneTapBottomSheet") {
            let oAuthListWidgetView = self.vkid.ui(for: oAuthListWidgetConfig).uiView()
            if let control: UIControl = oAuthListWidgetView.findElements({
