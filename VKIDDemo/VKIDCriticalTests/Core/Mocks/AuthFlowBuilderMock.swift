...
final class AuthFlowMock: AuthFlow {
    internal var handler: ((
        _ presenter: UIKitPresenter,
        _ completion: @escaping AuthFlowResultCompletion
    ) -> Void)?

    init(handler: ((_: UIKitPresenter, _: AuthFlowResultCompletion) -> Void)? = nil) {
        self.handler = handler
    }

    func authorize(with presenter: UIKitPresenter, completion: @escaping AuthFlowResultCompletion) {
        self.handler?(presenter, completion)
    }
}

final class AuthFlowBuilderMock: AuthFlowBuilder {
    internal var webViewAuthFlowHandler: ((
        _ authContext: AuthContext,
        _ authConfig: ExtendedAuthConfiguration,
        _ appearance: Appearance
    ) -> any AuthFlow)?
    internal var authByProviderFlowHandler: ((
        _ authContext: AuthContext,
        _ authConfig: ExtendedAuthConfiguration,
        _ appearance: Appearance
    ) -> any AuthFlow)?
    internal var serviceAuthFlowHandler: ((
        _ authContext: AuthContext,
        _ authConfig: ExtendedAuthConfiguration,
        _ appearance: Appearance
    ) -> any AuthFlow)?
