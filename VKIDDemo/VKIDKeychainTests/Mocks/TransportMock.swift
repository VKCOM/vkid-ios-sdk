...
final class TransportMock : VKAPITransport {
    ...
    func execute<T>(
        ...
        completion: @escaping (Result<T, VKIDCore.VKAPIError>) -> Void
    ) where T : VKIDCore.VKAPIResponse {
        callbackQueue.async { [weak self] in
            completion(self?.responseProvider?(request).map { $0 as! T }
                .mapError { $0 } ?? .failure(.noResponseDataProvided))
        }
    }
}
