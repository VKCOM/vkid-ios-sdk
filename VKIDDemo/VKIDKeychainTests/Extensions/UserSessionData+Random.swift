...
extension UserSessionData {
    static func random(
        ...
        accessTokenExpirationDate: Date = Date()
    ) -> UserSessionData {
        ...
        return UserSessionData(
            ...
            refreshToken: refreshToken ?? self.makeRefreshToken(userId: userId),
            idToken: self.makeIdToken(userId: userId),
            user: withUserData ? self.makeUser(userId: userId) : nil,
            serverProvidedDeviceId: UUID().uuidString
        )
    }

    private static func makeIdToken(userId: UserID) -> IDToken {
        IDToken(
            userId: userId,
    ...
    private static func makeAccessToken(
        userId: UserID,
    ...
    private static func makeUser(userId: UserID) -> User {
        User(
            id: userId,
