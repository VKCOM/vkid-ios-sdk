...
public struct GroupSubscriptionSheet: AsyncUIViewControllerElement {
    ...
    public func _uiViewController(
        ...
        func refreshAccessToken() {
            if !isRefreshedAccessToken.wrappedValue {
                self.accessTokenProvider.getAuthorizer(
                    needToRefresh: true
                ) { result in
                    switch result {
                    case .success(let authorization):
                        isRefreshedAccessToken.wrappedValue = true
        ...
        func completionHandler(
            ...
            case .success((
                ...
                completion(.success(self.createViewController(
                    groupInfo: groupInfo,
    ...
    private func createViewController(
        ...
        let viewController = GroupSubscriptionContentViewController(
            groupConfiguration: .init(
                groupId: Int(self.subscribeToGroupId) ?? 0,
                            ...
                            vkid.rootContainer.groupSubscriptionService.subscribeToGroup(
                                ...
                                case .success(let success):
                                    if success {
                                        ...
                                        subscribeCompletion(.success(false))
                                    }
                                case.failure(let error):
